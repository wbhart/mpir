...
// returns smallest d such that  d|N  start <= d < stop  d!=1 , if no such d exists return 0
// the name trial_division implies a method , perhaps call it mpz_smallest_divisor
...
ASSERT(start%2!=0);ASSERT(start%3!=0);
dd=2;if(start%6==1)dd=4;
for(i=start;i<stop;i+=dd,dd=6-dd)if(mpz_divisible_ui_p(N,i))return i;
return 0;}
