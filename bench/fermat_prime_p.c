...
// Going form k to k+1 we expect the runtime to increase by a factor of 4+epsilon , So runtime=A*4^k
int fermat_prime_p(unsigned long k)
{unsigned long i,k2;int c;mp_size_t n;mp_ptr tp,xp,yp,sp;

if(k>=BITS_PER_ULONG)k=BITS_PER_ULONG-1;// this should force a out of memory rather than some sort of crash
if(k==0)return 1;
k2=1;k2<<=k;// k2=2^k
...
xp=tp;yp=tp+2*n;
MPN_ZERO(xp,n);xp[0]=3;c=0;
for(i=1;i<k2;i++)
   {if(c!=0)c=3;// as we are squaring , dont need to do this , as for Pepin test it doesn't matter
